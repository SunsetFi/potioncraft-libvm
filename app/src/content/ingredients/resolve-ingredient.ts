...
export async function resolveIngredient(id: IngredientId): Promise<IngredientEntry | null> {
  const ingredients = await resolveIngredients();

  const idLower = id.toLowerCase();

  const ingredient = ingredients.find((ingredient) => ingredient.id.toLowerCase() === idLower);
